...
string AppendDirection(string s, int d) {
    switch (d)
    {
    case 0:  s += 'D'; break;
    case 1:  s += 'R'; break;
...
void bfs() {
    ...
    queue<pair<pair<pair<int, int>, pair<int, int>>, pair<bool, pair<int, string>>>> q;
    ...
    while (!q.empty())
    {
        ...
        int counting = q.front().second.second.first; // 판 움직인 횟수
        string move_recode = q.front().second.second.second;
        q.pop();
        ...
        for (int i = direct; i < 4; i += 2) { //방향: 0-북 / 1-동 / 2-남 / 3-서
            ...
            if (red_in_hole && !blue_in_hole) { //성공!
                cout << counting << " " << AppendDirection(move_recode, i);
                return;
            }
                ...
                q.push({ {red_next, blue_next} ,{ !direct , {counting + 1,AppendDirection(move_recode,i)}} });
            }
        }
    }
    cout << -1;
